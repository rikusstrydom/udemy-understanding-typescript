...
function validate(input: IValidatable) {
  let isValid = true;
  if (input.required) {
...
class ProjectInput extends Component<HTMLDivElement, HTMLFormElement> {
  ...
  private gatherUserInput(): [string, string, number] | void {
    ...
    const validateTitleConfig: IValidatable = { value: enteredTitle, required: true };
    const validateDescConfig: IValidatable = { value: enteredDescription, required: true, minLength: 5 };
    const validatePeopleConfig: IValidatable = { value: +enteredPeople, required: true, min: 1, max: 5 };
